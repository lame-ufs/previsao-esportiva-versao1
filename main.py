...
if pagina == 'Previsão Brasileirão Série A':
    ...
    times = ["Atlético-GO","Athletico-PR","Atlético-MG","Botafogo","Bahia", "Cruzeiro","Cuiabá","Criciúma","Corinthians",
    ...
    if st.button('Calcular Probabilidades'):
      if time1 not in full['time'].unique() or time2 not in full['time'].unique():
        ...
        for time in [time1, time2]:
            gols_feitos_casa = list(map(int, eval(full.loc[full['time'] == time, 'gf_casa'].values[0])))[:6][::-1]
            gols_sofridos_casa = list(map(int, eval(full.loc[full['time'] == time, 'gs_casa'].values[0])))[:6][::-1]
            gols_feitos_fora = list(map(int, eval(full.loc[full['time'] == time, 'gf_fora'].values[0])))[:6][::-1]
            gols_sofridos_fora = list(map(int, eval(full.loc[full['time'] == time, 'gs_fora'].values[0])))[:6][::-1]
        ...
        imagens = {"Vasco": "https://logodownload.org/wp-content/uploads/2016/09/vasco-logo-4.png",
                   ...
                   "Atlético-GO": "https://logodownload.org/wp-content/uploads/2017/02/atletico-goianiense-logo-4.png",
                   "Athletico-PR": "https://logodownload.org/wp-content/uploads/2016/10/atletico-paranaense-logo-escudo-768x768.png",
                   "Atlético-MG": "https://upload.wikimedia.org/wikipedia/commons/thumb/2/27/Clube_Atl%C3%A9tico_Mineiro_logo.svg/1810px-Clube_Atl%C3%A9tico_Mineiro_logo.svg.png",
