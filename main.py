    ...
    if st.button('Calcular Probabilidades'):
      if time1 not in full['time'].unique() or time2 not in full['time'].unique():
        ...
        for time in [time1, time2]:
            gols_feitos_casa = list(map(int, eval(full.loc[full['time'] == time, 'gf_casa'].values[0])))[::-1]
            gols_sofridos_casa = list(map(int, eval(full.loc[full['time'] == time, 'gs_casa'].values[0])))[::-1]
            gols_feitos_fora = list(map(int, eval(full.loc[full['time'] == time, 'gf_fora'].values[0])))[::-1]
            gols_sofridos_fora = list(map(int, eval(full.loc[full['time'] == time, 'gs_fora'].values[0])))[::-1]
